...
工作名：`deepquery`（可自定，避免叫"智能客服/知识库问答"这类已被看腻的名字）。
...
### 8.1 必背问答：SQL 是怎么生成的（面试第一问，背熟）

**问："你的 SQL 是怎么生成的？"**——五步答：

1. **组装 prompt**：角色规则（只许单条 SELECT、方言、思路+```sql 格式）+ 运行时自省的
   schema 上下文（表/字段/中文注释/3 行样例数据——样例让模型知道 city 存的是"上海"
   不是"Shanghai"）+ BM25 检索命中的业务字典/few-shot/用户记忆 + 问题本身。
2. **LLM 翻译**：模型预训练读过海量 SQL，本质是做一次"中文→SQL"的翻译，输出
   一句话思路 + ```sql 代码块。
3. **提取**：正则抠出代码块。生成到此结束——生成这步就是 LLM 干的。
4. **守卫**：sqlglot 解析成语法树检查（单 SELECT/表白名单/强制 LIMIT），
   过了才放行到只读连接执行。
5. **兜底**：执行报错→结构化分类→连同报错喂回模型定向重写，最多 3 轮，
   仍失败则如实降级。

关键认知：**模型从头到尾没有"连接"数据库**——它只读到一段描述表结构的文字；
真正碰库的永远是我们的代码，且戴着三层只读手铐。

**问："这不就是调 API 让模型写 SQL 吗？"**——标准答案：

> "对，生成这一步就是 LLM，这也是行业标准做法——Quick BI、PowerBI Copilot 的核心
> 同样是一次模型调用，没有谁为此重新发明模型。我的工作不是发明'生成'，而是让'生成'
> 变得可用：输出不可信，用语法树守卫和三重只读关住它；会出错，用错误分类驱动的循环
> 让它自己修；口径有歧义，用检索式知识注入对齐它；效果要证明，建评测集跑出带置信
> 区间的数字。模型是引擎，我造的是整辆车——引擎谁都买得到，车不是。"

一句压场的话：**"调用 LLM 从来不丢人，丢人的是只调用 LLM。"**
玩具和产品用的是同一次 LLM 调用，差的是调用外面那一整圈——守卫、自纠错、
口径注入、防幻觉、评测、成本控制——这一整圈就是本项目。

**问："不同模型效果如何？换个模型会怎样？"**——三段答：

1. **架构上模型是可插拔的**：走 OpenAI 兼容接口，换模型改一行配置；更重要的是
   评测体系让"换模型"从玄学变成实验——同一评测集跑一轮，20 分钟出对比表
   （EX/成本/延迟三列），选型靠数字不靠感觉。
2. **实测观察**（讲亲身跑出来的）：推理型模型延迟约为对话型的 8 倍
   （单题 39s vs 5s）、token 消耗约 3 倍，但在输出格式遵循上更细心（更少多余列）；
   对话型模型快而便宜，偶发的列冗余/口径偏差靠守卫和自纠错层兜住——
   **修复环对弱模型的收益更大**，这本身就是架构价值的证明。
3. **生产上这是成本-准确率权衡**：准确率差 2 个点、成本差 10 倍时，答案不是
   "用最强的"，而是便宜模型打底 + 失败时升级重试（模型路由）——方向已留在
   规划里，评测基建是做这件事的前提。
...
> 背景：此前规划过一个 OpsAgent（运维故障诊断 agent）两周冲刺方案，含手写 ReAct 内核 + 20 场景故障注入评测 harness。方向维持 deepquery 不变——OpsAgent 方案自己列出的三大瓶颈（一轮全量 eval 4-9 小时墙钟、确定性环境重置的 flaky 调试可能吞掉 2-3 天、8GB 内存需求且注入脚本可能搞挂宿主机）在 NL2SQL 方向上天然不存在（SQLite 复制文件即重置、单条评测毫秒级）。但它的评测方法论和工程纪律是通用资产，以下逐项移植：

| OpsAgent 组件 | 移植到 deepquery 的形态 |
|---|---|
